            ...
            <div class="col-lg-12 col-md-12">
                <div class="card">
                    <div class="card-header border-bottom">
                        <h3 class="card-title">{{ __('learn::custom.learn_add') }}</h3>
                    </div>
                    <div class="card-body">
                        <form action="{{ route('learn.store') }}" method="post" enctype="multipart/form-data" class="row g-3 needs-validation" novalidate>
                            <div class="col-md-6">
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea name="short_text" class="form-control" id="short_text" required rows="2">{{ old('short_text') }}</textarea>
                                <div class="invalid-feedback">لطفا متن کوتاه را وارد کنید</div>
                            </div>
                            <div class="col-md-12">
                                <label for="type" class="form-label">نوع آموزش</label>
                                <select name="type" class="form-control" required>
                                    <option value="text" @if(old('type') == 'text') selected @endif>متنی</option>
                                    <option value="video" @if(old('type') == 'video') selected @endif>ویدئو</option>
                                </select>
                                <div class="invalid-feedback">لطفا نوع آموزش را انتخاب کنید</div>
                            </div>
                            <div class="col-md-12">
                                <label for="body" class="form-label">متن کامل</label>
                                <textarea id="editor1" name="body" class="cke_rtl">{{ old('body') }}</textarea>
                            </div>
